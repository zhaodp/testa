<?php
/**
 * 订单详情
 * User: mtx
 * Date: 13-12-4
 * Time: 下午5:37
 * auther mengtianxue
 */
$month = date('Y-m', strtotime($params['daily_date']));
$driver_id = $params['driver_id'];
?>

<h1>VIP销费汇总表</h1>
<ul class="nav nav-tabs">
    <li>
        <?php
        $year_2012 = Yii::app()->createUrl('/finance/vip_trade_info',
            array('year' => '2012'));
        ?>
        <a href="<?php echo $year_2012; ?>">2012年Vip汇总</a>
    </li>

    <li>
        <?php
        $year_2013 = Yii::app()->createUrl('/finance/vip_trade_info',
            array('year' => '2013'));
        ?>
        <a href="<?php echo $year_2013; ?>">2013年Vip汇总</a>
    </li>

    <li>
        <?php
        $year_2014 = Yii::app()->createUrl('/finance/vip_trade_info',
            array('year' => '2014'));
        ?>
        <a href="<?php echo $year_2014; ?>">2014年Vip汇总</a>
    </li>

    <li>
        <?php
        $month_url = Yii::app()->createUrl('/finance/vip_group_driver',
            array('month' => $month));
        ?>
        <a href="<?php echo $month_url; ?>"><?php echo $month; ?>司机VIP收入</a>
    </li>
    <li class="active"><a href="javascript:void(0);"><?php echo $driver_id; ?>司机VIP收入详情</a></li>
</ul>


<div class="tab-content">
    <?php $this->widget('zii.widgets.grid.CGridView', array(
        'id' => 'knowledge-grid',
        'dataProvider' => $model->search(),
        'itemsCssClass' => 'table table-striped',
        'columns' => array(
            'driver_id',
            'order_id',
            'daily_date',
            'amount',
            'cast',
            'insurance',
            'Invoice_money',
            'balance',
        ),
    )); ?>
</div>