...
<div class="row-fluid">
    <table class="table table-bordered table-striped">
        ...
        foreach ($data as $v) {
            ...
            $url = Yii::app()->createUrl('/finance/daily_list', $arr);
            $start_time = $v->month_date . '-01';
            $start_balance = BDailyAccountReport::model()->getDailyMoney($start_time, 0);
            ...
            $end_balance = BDailyAccountReport::model()->getDailyMoney($end_time, 1);

            $back_status = BDailyAccountReport::model()->getDailyAccountNotStatus($v->month_date);
            if ($back_status) {
                $like = "是";
            } else {
                $like = "否";
