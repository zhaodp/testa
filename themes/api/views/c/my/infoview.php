...
if(!$customer_info || empty($customer_info)){
    $ret = array('code' => 2, 'message' => '没有找到客户信息');
    echo json_encode($ret);
    return;
}
...
$ecoins = CustomerMain::model()->getWealth($phone);
$balance = 0;
$company='';
$viptype="";
if($vip){
    ...
    if($data && isset($data['vipid']) && !empty($data['vipid'])){
        $viptype=$data['type'];
        $vipid=$data['vipid'];
		//解除在APP上VIP修改姓名的限制--如果用户是VIP，会重新给用户名赋值为VIP账号，而不是真正修改的customer名字
		//$name = $data['name'];
        $vip_info=Vip::model()->getPrimary($vipid);
        if($vip_info&&$data['type']>VipPhone::TYPE_VICE){
            /*$name = $vip_info->name;
