...
$reason_codes = isset($params['reason_codes']) ? trim($params['reason_codes']) : '';//司机评价原因，客户端传过来为1,2,3这样的key字符串
$no_star = isset($params['no_star']) ? trim($params['no_star']) : 0;//司机评价原因，客户端传过来为1,2,3这样的key字符串
$no_star_comment = Yii::app()->params['no_star_comment'];
EdjLog::info("c.order.comment, no start on or off:".$no_star_comment.", params:" .json_encode($params));

if(empty($no_star)||$no_star_comment!="on"){
    //参数有效性验证
    if (empty($token) || empty($level) || empty($order_id) || empty($driver_id)) {
        $ret = array(
            'code' => 2,
            'message' => '参数错误'
        );
...
if(empty($no_star)||$no_star_comment!="on"){
    ...
    if(!empty($reason_codes)){
        if(array_key_exists($level, CommentSms::$resonArray)){
            $reasons = CommentSms::$resonArray[$level];//对应具体级别具体的reason
            $reason_codesArr = explode(",",$reason_codes);
            foreach($reason_codesArr as $reson_key){
                $reson_key = $reson_key - 1;
                $reasonStr = isset($reasons[$reson_key]) ? $reasons[$reson_key]:array() ;
                if(is_array($reasonStr)){
                    $reson_strs .= (isset($reasonStr['detail']) ? $reasonStr['detail'] : '') .",";
                }
            }
            $reason = $reson_strs;
        }
    ...
    if($record_complaint){
        ...
        $data = array(
            'driver_id' => $driver_id,
            'opinion_id' => $customer_complian->attributes['id'],
    ...
    $command_data['reason_codes'] = $reason_codes;
    ...
    if($ret['code'] == 0){
        ...
        if($order){
            $share_activity = PageConfig::model()->getSharedInfoByOrderIdAndTriggerTime($order, PageConfig::TRIGGER_COMMENT);
    ...
    $command_data = array();
    EdjLog::info("c.order.comment, no start comment, params:".json_encode($params));
    if(!empty($reason_codes)){
        $reasonStr = OrderNoStarCommentConfigHandler::model()->getReason($reason_codes);
        if($reasonStr){
            $reason = $reasonStr;
        }
    }
    $command_data['order_id'] = $order_id;
    $command_data['level'] = 0;
    $command_data['content'] = $content;
    $command_data['reason'] = $reason;
    $command_data['sender'] = $validate['phone'];
    $command_data['driver_id'] = $driver_id;
    ...
    $command_data['reason_codes'] = $reason_codes;
    //为什么不走队列？ add by sunhongjing
    EdjLog::info("c.order.comment, no start comment, add content:".json_encode($command_data));
    $comment_sms = CommentSms::model()->addOrderCommand($command_data);
    $ret = array('order_id' => $order_id);
    switch($comment_sms){
        case 0:
