...
<div class="btn-group">
	...
	<?php $this->renderPartial('_online_search',array(
		'model'=>$model,'datelist'=>$datelist,'city_type'=>$city_type,'select_box_array'=>$select_box_array
	)); 
	?>
	</div><!-- search-form -->
</div>
...
$(function () {
	...
	foreach($city_worklog as $worklog){
		...
		$totalDriver = $worklog['data']['totalDriver'];
		
		$js =sprintf($js_template,$id,$id,$title,$timeLine,$totalDriver,$busyDriver,$freeDriver,$offlineDriver,$id);
		echo $js."\n";
	}
    ?>
    });
...
foreach ($citys as $city) {
    echo "<div id='container{$city}' style='height: 250px; margin: 0 auto;' class='span12'></div>\n";
}
