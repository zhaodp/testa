...
<?php $this->widget('zii.widgets.grid.CGridView', array(
	...
	'columns'=>array(
		...
		array(
			'name'=>'可否签约',
			'type'=>'raw',
			'value'=>'( $data->entrant_time == 0&& $data->complete==1&&$data->status==3) ? "可以签约" : "不能签约"',
		),
		array(
			'name'=>'取消',
			'type'=>'raw',		
...
?>
<center>
<input type="submit" value="确认签约" class="btn btn-success" id="submit_btn">
</center>
<script>
$(function(){
	...
	<?php 
		...
		foreach ($tmpData as $item){
			if($item['status']!=3||$item['entrant_time'] != 0||$item['complete']!=1){?>
	...
	$("#submit_btn").click(function(){
		...
		if(other_id.length!=0){ alert("目前有"+other_id.length+"位司机目前不能签约，请重新确认列表里的司机，");return false;}
		for(i=0;i<num;i++){
			if(i!=0){
		...
		//ajax
		$.ajax({
			'url':'<?php echo Yii::app()->createUrl('/recruitment/createdriverentry');?>',
			'data':'batch='+batch+'&id='+arrID+'&sms_content='+sms_content,
			'type':'get',
			'beforeSend':function(){
				$("#submit_btn").css("disabled","disabled");
			},
