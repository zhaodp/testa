...
<?php
    ...
    if (is_array($driver_info) && count($driver_info) && isset($right) && $right) {
    ...
    if(empty($right)) {
        echo '<div class="alert alert-success">';
        echo '<strong>不能操作其他城市的司机。</strong>';
        echo '</div>';
    }
    if(!is_array($driver_info)){
        echo '<div class="alert alert-success">';
        echo '<strong>无此司机信息，请确认报名流水号填写正确。</strong>';
        echo '</div>';
    }
    if (isset($driver_info) && $driver_info['recycle']) {
        ...
        echo '<div class="alert alert-error">该司机路考已经通过</div>';
    }elseif(isset($driver_info) && isset($open) && empty($open) && $driver_info['status'] != 2) {
    ...
    } elseif(is_array($driver_info) && count($driver_info) && $right){
?>
