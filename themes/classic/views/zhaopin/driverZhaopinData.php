...
function import_date(){
    ...
    var url = '<?php echo Yii::app()->createUrl('/zhaopin/importdriverzhaopin',array('batch'=>$_GET['batch']));?>';
	url += '&id='+arrID;
...
function sns_send(){
    ...
    if(sms_content !=''){
	    ...
	    if(arrID.length > 0){
		    $.ajax({
				'url':'<?php echo Yii::app()->createUrl('/zhaopin/sendSMS');?>',
				...
				'success':function(data){
					...
					if(data==1){
						alert("发送成功。");
						window.location.href="<?php echo Yii::app()->createUrl('/zhaopin/driverbatchadmin');?>";
					}
					else
						alert("操作失败！");
				},
