...
<?php
$tmp_english_name = 'bd_'.time();
$id = '';
$name = '';
// $english_name = 'simple-test-'. rand(1,99999);
$english_name = $model['english_name'];
$bonus_sn = '6586323444';//新客 29元
$bonus_sn2 = '1457318762';//老客   5元
$bonus_sn3 = '6677805083';//老客   9元
$bonus_num = '50000';
$bind_num = '';
...
$subsidy_type = '';
$subsidy_amount = '';
$data_send_begin_time = $model['data_send_begin_time'];
$data_send_end_time = $model['data_send_end_time'];
$channel = $model['english_name'];
$send_rate = '';
$send_mails = 'lihongyan@edaijia-inc.cn';
...
if(isset($model['id'])){
    ...
    $bonus_sn3 = isset($model['bonus_sn3']) ? $model['bonus_sn3'] : '';
    $bonus_num = isset($model['bonus_num']) ? $model['bonus_num'] : '50000';
    $bind_num = isset($model['bind_num']) ? $model['bind_num'] : '';
    $begin_time = isset($model['begin_time']) ? $model['begin_time'] : '';//todo
    ...
    $data_send_begin_time = isset($model['data_send_begin_time']) ? $model['data_send_begin_time'] : '';//todo
    ...
    $short_url = isset($model['short_url']) ? $model['short_url'] : '';
    $qrcode = isset($model['qrcode']) ? $model['qrcode'] : '';
    if(!empty($page_json)){
...
?>
    ...
    <body>
        ...
        <form class="form-create">
            <div class="wrapper">
                <section>
                    ...
                    <p style="display:none"><label>活动英文名称: </label><input placeholder="限15字以内英文、数字及_" name="english_name"  value="<?php echo $english_name;?>"></p>
                </section>
            </div>
            <div class="wrapper">
                <section>
                    ...
                    <fieldset>
                        <legend>文字部分</legend>
                        <p><label>页面标题: </label><input maxlength="10" required class="json-item" data-item="text_page_title"></p>
                        <p><label>页面副标题: </label><input maxlength="12" required class="json-item" data-item="text_subtitle"></p>
                        <p style="display:none"><label>参与按钮文案: </label><input required class="json-item" data-item="text_join" value="我要领劵"></p>
                        <p style="display:none"><label>分享按钮文案: </label><input required class="json-item" data-item="text_share" value="分享"></p>
                        <p class="flex" style="display:none"><label>活动规则: </label>
                            <textarea  required rows="4" class="json-item" data-item="text_rule">
                                    <p>仅限绑定后7日内使用，结算代驾费时会自动扣除。不与其他优惠累计，不设找零。</p>
                                    <ul>
                                        <li>每个手机号只可领取一次</li>
                                        <li>通过400下单不能使用</li>
                    ...
                    <fieldset style="display:none">
                        ...
                        <p><label>首页头部: </label>
                            <input type="file">
                            <input type="hidden" required class="json-item" data-item="img_top" value="<?php echo $static_host; ?>/core/event_modules/module3/img/bg.jpg">
                        </p>
                        <p><label>首页底部: </label>
                            <input type="file">
                            <input type="hidden" class="json-item" data-item="img_btm">
                            <label><input type="checkbox" class="use-empty" checked>留空</label>
                        </p>
                        <p><label>成功页头部: </label>
                            <input type="file" value="">
                            <input type="hidden" required class="json-item" data-item="img_top_suc" value="<?php echo $static_host; ?>/core/event_modules/module3/img/bg.jpg">
                            <label><input type="checkbox" class="use-first-top" checked>使用首页头部url</label>
                        </p>
                        <p><label>成功页底部: </label>
                            <input type="file">
                        ...
                        <p><label>结束页头部: </label>
                            <input type="file">
                            <input type="hidden" required class="json-item" data-item="img_top_over" value="<?php echo $static_host; ?>/core/event_modules/module3/img/bg.jpg">
                            <label><input type="checkbox" class="use-first-top" checked>使用首页头部url</label>
                        </p>
                        <p><label>结束页底部: </label>
                            <input type="file">
                            ...
                            <label><input type="checkbox" class="use-first-btm" checked>使用首页底部url</label>
                        </p>
                        <p><label>断网页底部: </label>
                            <input type="file">
                        ...
                        <p><label>活动背景色: </label><input required placeholder="例如#ffffff" size="6" class="json-item" data-item="color_common" value="#00a0e9" ></p>
                        <p><label>断网背景色: </label><input required placeholder="例如#ffffff" size="6" class="json-item" data-item="color_neterr" value="#00a0e9" ></p>
                ...
                <aside>
                    <h1>预览页面</h1>
                    <iframe id="previewIfr" src="<?php echo $static_host;?>/core/event_modules/module3/preview.html" width="320" height="568" ></iframe>
                    <p>
                        <label>选择页面: </label>
            ...
            <div class="wrapper">
                <section>
                    <h1>分享设置</h1>
                    <p class="flex"><label>分享标题: </label><input required class="json-item" data-item="text_share_tit" value=""></p>
                    <p class="flex"><label>分享描述: </label><input required class="json-item" data-item="text_share_dec" value=""></p>
                    <p><label>分享图片: </label><input type="file"><input required type="hidden" class="json-item" data-item="img_share"  value="<?php echo $static_host;?>/core/event_modules/module3/img/wx-share.png" todo></p>
                </section>
                <aside>
                    <h1>分享预览</h1>
                    <div class="share-preview-wrapper">
                        <h4 class="share-tit"></h4>
                        <img width="50" height="50"  src="<?php echo $static_host;?>/core/event_modules/module3/img/wx-share.png" >
                        <p class="share-dec"></p>
                    </div>
                </aside>
            </div>
            <div class="wrapper" style="display:none">
                <section>
                    ...
                    <fieldset>
                        <legend>业务信息</legend>
                        <p><label>新用户优惠券sn码: </label><input required name="bonus_sn" value="<?php echo $bonus_sn;?>"></p>
                        <p><label>老用户优惠券sn码(领过劵): </label><input required name="bonus_sn2" value="<?php echo $bonus_sn2;?>"></p>
                        <p><label>老用户优惠券sn码(未领劵): </label><input required name="bonus_sn3" value="<?php echo $bonus_sn3;?>"></p>
                    ...
                    <fieldset>
                        ...
                        <p><label>发送频率: </label>
                            <select required name="send_rate">
                                <option value="1" selected="selected"> <?php if($send_rate == 1){echo 'selected'; }?>>每天</option>
                                <option value="2" <?php if($send_rate == 2){echo 'selected'; }?>>每周</option>
                                <option value="3" <?php if($send_rate == 3){echo 'selected'; }?>>每月</option>
                            </select>
                        </p>
                        <p><label>收件人邮箱: </label>
                            <textarea required value="simple@edaijia-inc.cn" placeholder="多个邮件地址以;分隔" name="send_mails"><?php if(!empty($send_mails)){echo $send_mails; }?></textarea>
                        </p>
                    </fieldset>
                </section>
                <aside>
            ...
            <div class="wrapper">
                <section>
                    <h1>便捷操作</h1>
                    <p><label>短链接: </label><span><?php echo $short_url;?></span>
                    <p><label>二维码: </label><img id="qrcode" src="<?php echo $qrcode;?>" />
                    <p><label>更多二维码: </label></p>
                    <div id="qrcodes"></div>
                    <input type="hidden" name="short_url" value="<?php echo $short_url;?>">
        ...
        <script type="text/javascript" src="<?php echo $static_host;?>/core/event_modules/module3/js/admin.js"></script>
    </body>
</html>