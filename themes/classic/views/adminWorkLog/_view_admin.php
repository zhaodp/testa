    ...
    <thead>
        <tr>
            <th colspan="2">
                ...
                <?php echo CHtml::link('回复', array('adminWorkLog/reply','logId'=>$data->id), array('id'=>$updateId, 'style'=>'display:none;float:right;',
                    'onclick'=>'$("#log_reply_Iframe").attr("src",$(this).attr("href")).show();$("#log_reply").dialog("open");return false;'
                )); ?>
            </th>
        </tr>
    </thead>
    <tbody id="<?php echo $demoId; ?>" class="panel-collapse collapse <?php echo $index == 0 ? 'in' : 'hide' ?>">
        <tr>
            <td style="width:100px;" <?php echo !empty($replys) ? 'rowspan="2"' : ''; ?>>
                ...
                <small>时间：<?php echo $data->create_time; ?></small><br>
            </td>
            <td height="100%">
                <div class="log">
