...
<div class="span9">
    <div>
       选择加载公里数 <?php echo CHtml::radioButtonList('idel_max_distance',$data['idel_max_distance'],array(5000=>5,10000=>10),array('template'=>'{input}{labelTitle}','separator'=>'&nbsp')); ?>
        &nbsp;&nbsp;&nbsp;&nbsp;重新加载周围司机:
    ...
    <input class="btn" type="button" id="load_driver_50" onclick="load_driver(50);" value="加载50位司机" />
        <span>
            <p style="color:#ff0000">[<?php echo trim($data['city']);?>]该城市已开通app远程叫单服务，请勿加载10公里司机派单。</p>
        </span>
    </div>
...
<div class="span3">
    ...
    <form class="navbar-form pull-left span12">
        ...
        <input type="hidden" name="r" id="r" value="client/newmap"/>
...
var idel_count = 10;//加载司机数目 默认10个
...
function addMarkers(){
    ...
    req.idel_max_distance = $('input[name="idel_max_distance"]:checked').val();//选中加载的公里数
    ...
    return $.ajax({
        ...
        success: function (data) {
            if(data.code == 0){
                var driver_list = data.driverList;//返回司机坐标和工号
...
function add_group(queue_id, message){
    var max_distance = $('input[name="idel_max_distance"]:checked').val();//加载司机选中的公里数
    $.get("index.php", {r :'client/insertcart', queue_id: queue_id, user : message, cart : $("#cart").val(),max_distance : max_distance},
        function(data){
            $('#cart-menu').html( data );
        });
    cartShow();
