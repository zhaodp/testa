...
<?php 
    if ($useCloud) {
        $hojoLoginName = Yii::app ()->user->agent['agent_num'] . '@bjedj';
        $hojoLoginPassword = '123abc';
        if ($useCloud == 1) {
            $hojoLoginType = 'gateway';
        } else {
            $hojoLoginType = 'Local';
        }
        $cloudType = 'private';
    } else {
        $hojoLoginName = Yii::app()->user->agent['agent_name'];
        $hojoLoginPassword = Yii::app()->user->agent['password'];
        $hojoLoginType = 'Local';
        $cloudType = 'public';
    }
?>
...
hojo.addOnLoad(login);

function login() {
    var loginName = "<?php echo $hojoLoginName ?>";
    var password = "<?php echo $hojoLoginPassword ?>";
    var loginType = "<?php echo $hojoLoginType ?>";
    var cloudType = "<?php echo $cloudType ?>";
    
    icallcenter.logon.startLogon(loginName, password, loginType, cloudType);
}
