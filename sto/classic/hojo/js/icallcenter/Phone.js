...
hojo.declare("icallcenter.Phone", null, {
	...
	constructor: function(config) {
        ...
        console.debug("开始初始化控件");
		this._loadConfig(config);
		console.debug("接听类型："+this.extenType);
        if (this.extenType == "sip") {    
            this._loadSipPhone();
        }
    ...
    _loadSipPhone: function() {
        sipPhone.StrUser = this.sipNo;
        sipPhone.StrPassword = this.sipPassword;

        sipPhone.StrLine = this.sipNo;
        sipPhone.StrServer = this.pbx_ex_ip;
        sipPhone.InitSipPhone();
    },
    ...
    _exit: function(loginName, removeQueue) {
    	 ...
    	 var returnUrl = "index.php?r=client/callphone";
    	 console.debug("注销ASS"+removeQueue);
...
/*
        	         		if(comments == "ukick" || comments == "ekick")
        	         			icallcenter.hojotools.error("您当前的会话已经失效,导致该问题的原因是别的坐席使用相同的帐号（或相同的分机）登录了");	
        	         		else
...
*/
    ...
    _relogin: function(){
    	...
    	var phoneJson = {
        		...
        		Exten:self.sipNo,
        		Secret:self.sipPassword,
        		LoginExten:self.extension,
        		DialoutPrefix:self.dialOutPrefix,
        		DidNum:self.didNum,
        		TransferPrefix:self.xferPrefix,
		...
		hojo.io.script.get({
            ...
            load: function(response, ioArgs) {
                ...
                if (!_response.Succeed) {
                    ...
                    if(code){
                    	if(code == 601) {
                    		icallcenter.hojotools.error("您的账户通话坐席登录数已达最大或者已经到期,请使用无通话方式登录或联系管理员");	
                    	} else if(code == 602) {
                    		icallcenter.hojotools.error("您的账户无通话坐席登录数已达最大或者已经到期,请使用软电话/网关/直线方式登录或联系管理员");
                    	} else {
    ...
    answer: function() {
    	console.debug("接听电话："+this.extenType);
        var self = this;
    	// summary:
        //  软电话应答
        if (this.extenType == "sip") {
            try {
                sipPhone.AnswerCall();
                console.debug("接听电话成功");
            } catch (e) {
                console.debug("应答失败:" + e);   
                return ERR_GENERAL;
            }
       } else {
            console.error("当前接入方式不支持该操作");
            return ERR_NOT_SUPPORT_ACTION;
       }       
       return SUCCESS;
    },
    ...
    pick: function (userId) {
    	console.debug("签入坐席"+userId);
    	var self = this;
    	var peer = self.monitorPeers[userId];
    	if(peer == null || peer.localNo == null || peer.localNo == ""){
    		icallcenter.hojotools.error("不能对没有直线号码的坐席做签入操作");
    		//alert("不能对没有直线号码的坐席做签入操作");
    		return;
    	}
