...
// e.g. ../yiic guangfaBank --bonus_id=7164 --calc_type=detail
//
class guangfaStatCommand extends LoggerExtCommand
{
    private function calc_7164($calc_type)
    {
        try {
            $start = strtotime('2015-03-01');
            $end = strtotime('2015-03-31');
            $data = array();
    ...
    public function actionIndex($bonus_id, $calc_type)
    {
        if ($bonus_id == 95508) {
            $this->calc_95508();
        } else if ($bonus_id == 7164 && $calc_type == 'detail') {
            $this->calc_7164('detail');
        } else if ($bonus_id == 7164 && $calc_type == 'summary') {
            $this->calc_7164('summary');
