...
class GoogleAuthenticator
{
    ...
    {
        $cache_key = $secret . $code;
        //此 code 在有效期内只能使用一次
        if(true === RedisHAProxy::model()->get($cache_key)){
            return false;
        }
        $time = floor(time() / 30);
        for ( $i = 0 - $window; $i <= $window; $i++) {
            if ($code == self::getCode($secret,$time + $i)) {
                //code 在 $window * 60 时间内不可再次使用
                RedisHAProxy::model()->set($cache_key, true, $window * 65);
                return true;
            }
        }
    ...
    public static function base32Decode($str)
    {
        ...
        if (!preg_match('/^[A-Z2-7]+$/', $str)) {
            EdjLog::error('Invalid characters in the base32 string: '. $str);
