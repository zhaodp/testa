 ...
 */
class DailyOnlineReport extends ReportActiveRecord
{
...
//	}
	...
	public function rules()
	{
		...
		return array(
			array('driver_id, record_date, one, two, three, four, five, six, seven, eight, nine, ten, eleven, twelve, thirteen, fourteen, fifteen, sixteen, seventeen, eighteen, nineteen, twenty, twentyone, twentytwo, twentythree, twentyfour, year, month, day, created', 'required'),
			...
			array('id, driver_id, record_date, one, two, three, four, five, six, seven, eight, nine, ten, eleven, twelve, thirteen, fourteen, fifteen, sixteen, seventeen, eighteen, nineteen, twenty, twentyone, twentytwo, twentythree, twentyfour, year, month, day, created', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('record_date',$this->date,true);
     ...
     * @return bool
     */
    public function insertData($record_date, $driver_id, $online_data) {
        ...
        if (is_array($online_data) && count($online_data)) {
            ...
            $_model->record_date = $record_date;
    ...
    /**
     * Get the driver online for a city and a specified day
     * @author qiujianping@edaijia-staff.cn
     * @created 2014-10-08
     * @params city_id city_id
     * @params date date like 2014-10-08
     */
    public function getOnlineDriverNumForCityByDay($city_id, $date) {
	$date_value = $this->getYMDByDate($date);
	$year = $date_value['year'];
	$month = $date_value['month'];
	$day = $date_value['day'];

	// Get city prefix 
	// Here we get city_prefix instead of city id for this table 
	// does not contains city_id
	$city_sql = "SELECT name FROM t_dict WHERE code={$city_id} AND dictname='city_prefix'";
	$command = Yii::app()->db_readonly->createCommand($city_sql);
	$data = $command->queryRow();
	if(count($data) == 0) {
	    return 0;
	}
	$city_prefix = $data['name'];

	$select_str = "select count(*) as online_count from t_daily_online_report where year={$year} and month={$month} and day={$day}";
	$select_str = $select_str." and driver_id like '{$city_prefix}%'";
	$select_str = $select_str." and (one+two+three+four+five+six+seven+eight+nine+ten+eleven+twelve+thirteen+fourteen+fifteen+sixteen+seventeen+eighteen+nineteen+twenty+twentyone+twentytwo+twentythree+twentyfour)>0";
        $command = Yii::app()->dbreport->createCommand($select_str);
        $data = $command->queryRow();
    ...
    public function getDriverOnlineInfo($driver_id, $year, $month) {
        $sql = "SELECT * FROM t_daily_online_report WHERE driver_id='{$driver_id}' AND year={$year} AND month={$month} ORDER BY record_date desc";
        $command = Yii::app()->dbreport->createCommand($sql);
        $data = $command->queryAll();
        return $data;
    }
