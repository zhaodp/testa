...
//		$html_text = $alipaySubmit->buildForm($para_temp, $this->alipay_gateway_new, "get", $button_name,$this->alipay_config);
		$html_text = $this->buildUrl($para_temp);
		return $html_text;
	}
	...
	function query_timestamp() {
		$url = $this->alipay_config['gateway']."service=query_timestamp&partner=".trim($this->alipay_config['partner']);
		$encrypt_key = "";
		$doc = new DOMDocument();
		$doc->load($url);
	...
	function buildForm($para_temp, $method, $button_name) {
		//待请求参数数组
		$para = $this->buildRequestPara($para_temp);
		$sHtml = "<form id='alipaysubmit' name='alipaysubmit' action='".$this->alipay_config['gateway']."_input_charset=".trim(strtolower($this->alipay_config['input_charset']))."' method='".$method."'>";
     ...
     * @return 访问支付宝的URL地址
     */
	function buildUrl($para_temp) {
		return $this->alipay_config['gateway'].$this->buildRequestParaToString($para_temp);
	}
	...
	function sendPostInfo($para_temp) {
		...
		//待请求参数数组字符串
		$request_data = $this->buildRequestParaToString($para_temp,$this->alipay_config);
		//请求的url完整链接
		$url = $this->alipay_config['gateway'] . $request_data;
		//远程获取数据
	...
	function getResponse($notify_id) {
		...
		$veryfy_url = '';
		if($transport == 'https') {
			$veryfy_url = $this->alipay_config['https_verify_url'];
		}
		else {
			$veryfy_url = $this->alipay_config['http_verify_url'];
		}
